...
acicd_a429_signal_int_charact::acicd_a429_signal_int_charact(sql_database_manager *database_manager) : acicd_element (database_manager)
{
    ...
    test_query=QString("SELECT rowid FROM %1 WHERE (min=:min) AND (max=:max) AND (unit=:unit) AND (coding_type=:coding_type) AND (resolution=:resolution);").arg(DB_table_name);

    create_table_query=QString("\
             CREATE TABLE IF NOT EXISTS [%1](\
                [min] NUMERIC CHECK (min != ''),\
                [max] NUMERIC CHECK (max != ''),\
                [unit]  VARCHAR( 10 ) CHECK (max != ''),\
                ...
                [non_linear_scale] VARCHAR( 45 ) CHECK (coding_type != ''),\
                    CONSTRAINT unique_combinaison PRIMARY KEY (min, max,unit, coding_type,resolution) ON CONFLICT IGNORE\
                );\n\
            ").arg(DB_table_name);
